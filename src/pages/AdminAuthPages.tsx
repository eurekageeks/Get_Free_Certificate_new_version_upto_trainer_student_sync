    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-400 text-sm">Sign in to manage your platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="admin@skillforge.io"
            />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          ...
          <button onClick={() => router.navigate('/admin/register')} className="text-teal-400 font-medium hover:underline">
            Register
          </button>
        </p>
...
export function AdminRegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Set the user's role to admin via secure edge function
    const userId = (await supabase.auth.getUser()).data.user?.id;
    if (userId) {
      const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
      const anonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;

      const res = await fetch(`${supabaseUrl}/functions/v1/admin-register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${anonKey}`,
        },
        body: JSON.stringify({ admin_code: adminCode, user_id: userId }),
      });
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-400 text-sm">Set up your administrator access</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1.5">Admin Access Code</label>
            <input
              type="text"
              value={adminCode}
            ...
            />
            <p className="mt-1 text-xs text-gray-500">Default code: <code className="text-teal-400 bg-gray-800 px-1.5 py-0.5 rounded">SKILLFORGE_ADMIN_2024</code></p>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          ...
          <button onClick={() => router.navigate('/admin/login')} className="text-teal-400 font-medium hover:underline">
            Sign in
          </button>
        </p>
      </div>
